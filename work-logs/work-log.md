...
### 벽 모드 UX 개선
- 벽 선택 시 + 버튼 깜박이기
- 축 라벨 거꾸로 보이는 버그 수정 (90° 초과 시 180° 보정)
- 벽 모드에서도 참조 이미지 버튼/페이지 전환 동작
- 참조 이미지에서 바닥/벽 탭 시 작업 화면 복귀
- 벽 모드 재진입 시 이전 선택 벽 복원 (두 벽 하이라이트 생략)
- 상단바 빨간 왼벽/오른벽 버튼 제거 (맵에서 직접 탭)
- 벽에 그리드 점선 추가 (타일 단위)
- 수정 파일: `lib/screens/placement_screen.dart`, `lib/widgets/grid_painter.dart`

### 벽 선택 시 + 버튼 깜박이기 (상세)
- 벽 모드에서 벽 탭 후 + 버튼 깜박여서 사물 추가 유도
- 수정 파일: `lib/screens/placement_screen.dart`

### 축 라벨 거꾸로 보이는 버그 수정
- Z축(왼쪽 아래) 라벨 회전 150° → 글자 뒤집힘
- 90° 초과 시 180° 보정하여 항상 읽기 쉬운 방향 표시
- 수정 파일: `lib/widgets/grid_painter.dart`

### v1.6.0 iOS + Android 배포
- 버전: 1.6.0+12
- iOS: App Store Connect 업로드 성공
- Android: Google Play 비공개 테스트(alpha) 업로드 성공
- Git 태그: `v1.6.0`
- 주요 변경사항:
  - 벽 명칭: 뒷벽 → 오른벽
  - 벽 가구 드래그 개선 (벽 모드 Y, 바닥 모드 방 안쪽)
  - 벽 가구 복제 같은 벽 유지
  - 벽 선택 안내 팝업
  - JSON 가져오기 도움말 + 벽 가구 예시
  - 반대벽까지 바닥 점선 가이드
  - 크기 수정 비율 유지 기본값 off
